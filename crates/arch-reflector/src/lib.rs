...
pub async fn run(options: &Cli) {
    ...
    let when = Utc::now();
    let status = get_mirror_status(10, 10, &options.url, &cache_file)
        .await
        .unwrap();

    if options.list_countries {
        list_countries(&status).await;
        return;
    }
...
pub fn filter_status<'a>(
    ...
    status.urls.iter().filter(move |mirror| {
        ...
        if !filters.country.is_empty() {
            if !filters.country.contains(&mirror.country)
...
pub async fn list_countries(status: &Status) {
